...
variable "static_ip" {
  description = "Static IP address for the termix container (without CIDR, e.g. 192.168.1.70)"
  type        = string
}
...
variable "memory_dedicated" {
  description = "Dedicated RAM in MB for the termix container"
  type        = number
  default     = 2048
}

variable "memory_swap" {
  description = "Swap in MB for the termix container"
  type        = number
  default     = 1024
}
