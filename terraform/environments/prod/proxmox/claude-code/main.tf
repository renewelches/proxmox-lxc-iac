...
resource "proxmox_replication" "claude-code" {
  count    = var.enable_replication ? 1 : 0
  id       = "${proxmox_virtual_environment_container.claude-code-container.vm_id}-0"
  type     = "local"
...
resource "local_file" "ansible_inventory" {
  content = templatefile("${path.module}/../../../../../ansible/inventory/prod/proxmox/claude-code/inventory.tpl", {
    ip = split("/", proxmox_virtual_environment_container.claude-code-container.initialization[0].ip_config[0].ipv4[0].address)[0]
  })
  filename = "${path.module}/../../../../../ansible/inventory/prod/proxmox/claude-code/inventory.ini"
}
