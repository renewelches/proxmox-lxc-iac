...
variable "static_ip" {
  description = "Static IP address for the Forgejo container (without CIDR, e.g. 192.168.86.163)"
  type        = string
}
...
variable "forgejo_domain" {
  description = "Domain name for the Forgejo instance (e.g., forgejo-dev.example.com)"
  type        = string
}
